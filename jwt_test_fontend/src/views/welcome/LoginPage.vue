...
import {loginApi} from "@/api/welcome/welcome.js";
import {storeAccessToken} from "@/utils/accessTokenUtils.js";
    ...
    if (isValid) {
      loginApi(form.username, form.password).then(response => {
        if (response.code == 200) {
          ...
          storeAccessToken(form.remember, data.token, data.expire);
          router.push("/index")
        }
      })
    }
