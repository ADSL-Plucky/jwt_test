...
import {askCodeApi, verifyAccountApi, resetConfirmApi, resetPasswordApi} from "@/api/welcome/welcome.js";
...
// 获取邮箱验证码
const validateEmail = () => {
  verifyAccountApi(form.email).then((response) => {
    if (response.code == 200) {
      coldTime.value = 60
      askCodeApi(form.email, "reset")
          .then((res) => {
            if (res.code == 200) {
...
const confirmReset = () => {
  formRef.value.validate((isValid) => {
    if (isValid) {
      resetConfirmApi(form.email, form.code)
          .then((response) => {
            if (response.code == 200) {
              active.value++
            }
...
const doReset = () => {
  formRef.value.validate((isValid) => {
    if (isValid) {
      resetPasswordApi(form.email, form.code,form.password)
          .then((response)=>{
            if(response.code==200){
